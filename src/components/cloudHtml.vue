...
<script>
	...
	import trees from './jyf-parser/trees';
	var cache = {},
		// #ifdef MP-WEIXIN || MP-TOUTIAO
		fs = uni.getFileSystemManager ? uni.getFileSystemManager() : null,
		// #endif
		Parser = require('./jyf-parser/MpHtmlParser.js');
	var dom;
	// 计算 cache 的 key
	function hash(str) {
	...
	var rpx = uni.getSystemInfoSync().screenWidth / 750,
		cfg = require('./jyf-parser/config.js');
	// #endif
	// #ifdef APP-PLUS-NVUE
	...
	export default {
		...
		watch: {
			html(html) {
				this.setContent(this.markdown ? marked(html) : html);
			}
		},
		mounted() {
			// 图片数组
